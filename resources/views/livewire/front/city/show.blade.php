<div class="min-h-screen pb-20 bg-base-200/30">
    ...
    <div class="relative w-full h-[50vh] md:h-[60vh]">
        <img src="{{ $city->getFirstMediaUrl('albums') }}" alt="{{ $city->name }}" class="w-full h-full object-cover" />
    ...
    <div class="container mx-auto px-4 -mt-20 relative z-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="card bg-base-100 shadow-xl overflow-hidden">
                    <div class="card-body p-6 md:p-10">
                        ...
                        <div class="border-b pb-6 mb-6">
                            ...
                            <h1 class="text-4xl md:text-5xl font-bold text-base-content">{{ $city->name }}</h1>
                        </div>

                        <div class="prose prose-lg max-w-none text-base-content/80">
                            <p class="leading-relaxed">
                                {{ $city->details ?? 'Experience the beauty of ' . $city->name . '. This destination offers unforgettable scenery and authentic local culture perfectly suited for your holiday photography sessions.' }}
                            </p>
                        </div>
                ...
                @if ($city->getMedia('photospots')->count() > 0)
                    <div class="card bg-base-100 shadow-xl">
                        <div class="card-body">
                            <h3 class="card-title text-2xl font-bold mb-6">Popular Photo Spots</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                @foreach ($city->getMedia('photospots') as $media)
                                    <div class="card bg-base-200 overflow-hidden group">
                                        <figure class="h-48">
                                            <img src="{{ $media->getUrl() }}" alt="Spot {{ $loop->iteration }}"
                                                class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                        </figure>
                                        <div class="card-body p-4">
                                            <h4 class="font-bold text-lg">
                                                {{ $media->getCustomProperty('location') ?? 'Spot #' . $loop->iteration }}
                                            </h4>
                                            <div class="flex items-center gap-2 text-xs text-gray-500">
                                                <x-mary-icon name="o-camera" class="w-4 h-4" />
                                                <span>Best for portraits</span>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
